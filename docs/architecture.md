...
- **掃描功能**: 支援拍照辨識條碼與手動輸入條碼。
- **入/出庫紀錄**: 先選擇入庫/出庫，再填寫人員與條碼，按下儲存後寫入入庫或出庫紀錄（含人員、時間戳）。
...
### 掃描流程
1. 使用者先選擇「入庫/出庫」，再填寫人員姓名。
2. 輸入或拍照辨識條碼，系統自動帶入產品名稱。
3. 點擊「儲存入出庫資料」寫入對應紀錄表，並自動寫入時間戳（年月日＋時分，不含秒）。
4. 即時庫存查詢表依公式自動更新。
...
### 寫入流程
1. 前端提交 `barcode`、`productName`、`quantity`、`type (in/out)`、`staff`。
2. 後端 GAS 寫入「入庫紀錄表」或「出庫紀錄表」，並補上時間戳。
3. 即時庫存查詢表自動更新。
